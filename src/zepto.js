...
var Zepto = (function () {
    ...
    //当需要给tr,tbody,thead,tfoot,td,th设置innerHTML的时候，需要用其父元素作为容器来装在HTML字符串
        containers = {
    ...
    //domready正则   当DOM ready的时候， document会有以下三种状态的一种
    ...
    zepto.matches = function (element, selector) {
        //严谨性判断，另 nodeType 属性返回以数字值返回指定节点的节点类型。  一共12种，1代表元素，2属性，3元素或属性中的文本内容
         ...
         * 再通过selector来查找这个div下的节点集，再判断给定element是否在节点集中，
         * 如果在，则返回一个非0的数字
         */
        // fall back to performing a selector:
        var match, parent = element.parentNode, temp = !parent
        //当element没有父节点，那么将其插入到一个临时div里边
     ...
     */
    function type(obj) {
        //obj为null或者undefined时，直接返回‘null’或‘undefinded’
    ...
    /**
     * 清除给定的参数中的null或undefined，注意 0 == null, '' == null 为false
     * @param array
     * @returns {Array.<T>}
     */
    ...
    /**
     * 数组去重，如果该条数据在数组中的位置与循环的索引值不相同，则说明数组中有与其值相同的值
     * @param array
     * @returns {Array.<T>}
     */
    uniq = function (array) {
    ...
    /**
     *给需要的样式后面增加px单位，除了cssNumber中指定的
     * @param name
     * @param value
     * @returns {*}
     */
    ...
    function defaultDisplay(nodeName) {
        ...
        if (!elementDisplay[nodeName]) {  //缓存里不存在
            ...
            display == "none" && (display = "block") //当display等于none时，设置其值为block,???
    ...
    //该方法模仿jquery $.fn.init   作为一个css选择器和一个可选的上下文（处理特殊情况）
    zepto.init = function (selector, context) {
        ...
        // 优化字符串选择器
            ...
            // 如果是一个HTML片段，从它创建一个节点
            if (selector[0] == '<' && fragmentRE.test(selector))
                dom = zepto.fragment(selector, RegExp.$1, context), selector = null
            // If there's a context, create a collection on that context first, and select
            // nodes from there
            //如果有上下文，首先创建上下文集合，然后选择那里节点
    ...
    $.extend = function (target) {
        ...
        if (typeof target == 'boolean') { //当第一个参数为boolean类型的值时，标示是否深度扩展
            deep = target
